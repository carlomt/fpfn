  ...
  for(int i=1; i<=steps; i++)
    {
      ...
      int THRbest=findBestTHR(nub0,nut0,time,MAXFP,MAXFN);
  		//if(canDetect(nub0,nut0,time,&THRbest,MAXFP,MAXFN)){
		//	printf("with t=%lf tumor can be detected with THR=%d\n",time,THRbest);
		//}else {
		//	printf("cannot detect with t=%lf\n",time);
		//}
	...
	{
	  if(fp<=MAXFP && fn<=MAXFN && fp>0 && fn>0)
	    {
